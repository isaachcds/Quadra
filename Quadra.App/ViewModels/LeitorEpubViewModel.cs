...
public partial class LeitorEpubViewModel
    : ObservableObject, IQueryAttributable
{
        ...
        IDictionary<string, object> query)
    {
        ...
        if (value is not ObraBiblioteca libraryItem)
            return;

        Item = libraryItem;
        ContentRoot = _leitorEpubService.ObterRaizConteudo(libraryItem);
    ...
    private async Task CarregarLivroAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var capitulos =
                await _leitorEpubService.CarregarCapitulosAsync(
                    Item,
                    cancellationToken);
